$(document).ready(function () {
  ...
  $("#addNewVehicle").click(function () {
    if (cusCode != "") {
      $("#vehiclemodal .modal-content").load(
        "../customer/loadmodal_vehicleadd/" + cusCode,
        function (result) {
          $("#vehiclemodal").modal({
            show: true,
            backdrop: "static",
  ...
  function loadSalesInvoiceCustomerPaymentDatatoGrid(resultData) {
    ...
    if (resultData.si_hed) {
      ...
      var invlink =
        baseUrl + "/Salesinvoice/view_sales_invoice_pdf/" + Base64.encode(poNo);
      $("#lastInvoice").html(
        "<a href='" + invlink + "'>Last Invoice - " + poNo + "</a>"
      );
      ...
      for (var i = 0; i < resultData.si_dtl.length; i++) {
        ...
        $("#tbl_item tbody").append(
          "<tr ri=" +
            ...
            i +
            " proCode='" +
            resultData.si_dtl[i].ProductCode +
            "' uc='" +
            resultData.si_dtl[i].PO_Type +
            "' qty='" +
            resultData.si_dtl[i].PO_Qty +
            "' vatunit_price='" +
            ...
            resultData.si_dtl[i].PO_UnitPrice +
            "' upc='" +
            resultData.si_dtl[i].PO_UPC +
            "' caseCost='" +
            resultData.si_dtl[i].PO_UnitCost * resultData.si_dtl[i].PO_Qty +
            "' isSerial='" +
            resultData.si_dtl[i].PO_IsVat +
            "' serial='" +
            resultData.si_dtl[i].PO_IsVat +
            "' discount_percent='" +
            resultData.si_dtl[i].PO_DisPercentage +
            "' cPrice='" +
            resultData.si_dtl[i].PO_UnitCost +
            "' pL='" +
            1 +
            "' fQ='" +
            0 +
            "' nonDisTotalNet='" +
            resultData.si_dtl[i].PO_TotalAmount +
            "' netAmount='" +
            resultData.si_dtl[i].PO_NetAmount +
            "' proDiscount='" +
            resultData.si_dtl[i].PO_DisAmount +
            "' proName='" +
            resultData.si_dtl[i].PO_ProName +
            "'   isvat='" +
            resultData.si_dtl[i].PO_IsVat +
            "' isnbt='" +
            resultData.si_dtl[i].PO_IsNbt +
            "' nbtRatio='" +
            resultData.si_dtl[i].PO_NbtRatio +
            "' proVat='" +
            resultData.si_dtl[i].PO_VatAmount +
            "' proNbt='" +
            resultData.si_dtl[i].PO_NbtAmount +
            "'  salesPerson='" +
            resultData.si_dtl[i].PO_IsVat +
            "' >" +
            ...
            i +
            "'>" +
            accounting.formatNumber(resultData.si_dtl[i].PO_Qty) +
            "</td>" +
            "<td class='text-right'>" +
            accounting.formatNumber(resultData.si_dtl[i].PO_UnitPrice) +
            "</td>" +
            "<td class='text-center'>" +
            resultData.si_dtl[i].PO_DisPercentage +
            "</td>" +
            "<td class='text-right' >" +
            accounting.formatMoney(resultData.si_dtl[i].PO_NetAmount) +
            "</td>" +
            "<td class='text-right' >" +
            0 +
            "</td>" +
      ...
      $("#netgrnamount").html(
        accounting.formatMoney(resultData.si_hed.PO_NetAmount)
      );
      $("#grndiscount").html(
        accounting.formatMoney(resultData.si_hed.PO_TDisAmount)
      );
      $("#totalgrn").html(accounting.formatMoney(total_amount));
      $("#totalVat").html(
        accounting.formatMoney(resultData.si_hed.POVatAmount)
      );
      $("#totalNbt").html(
        accounting.formatMoney(resultData.si_hed.PONbtAmount)
      );
      $("#shipping").html(accounting.formatMoney(0));
      $("#casAmount").html(
        "Advance Amount For Customer Order : ".concat(
          accounting.formatMoney(resultData.payment.payTotal)
        )
      );
      $("#advanceAmountTotal").html(
        "Advance Amount : ".concat(
          accounting.formatMoney(resultData.advancePayment.payAdvanceTotal)
        )
      );
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (resultData.advance) {
          ...
          advance_amount =
            parseFloat(advance_amount) + parseFloat(beforeAdvance);
        ...
        success: function (json) {
          ...
          if (resultData) {
            ...
            if (resultData.product) {
              ...
              loadVATNBT(
                resultData.product.IsTax,
                resultData.product.IsNbt,
                resultData.product.NbtRatio
              );
              ...
              loadProModal(
                ...
                resultData.product.UOM_Name,
                resultData.product.ProductVatPrice,
                resultData.serial.EmiNo
              );
            }
  ...
  $("#itemCode").autocomplete({
    ...
    select: function (event, ui) {
      ...
      discount_precent = ui.item.discount;
      $("#disPercent").val(discount_precent);
      $.ajax({
        type: "post",
  ...
  function add_products() {
    ...
    // }
      ...
      // serial and emi empty normal products here
  ...
  $("#tbl_item tbody").on("click", ".edit", function () {
    ...
    var disPrecent = parseFloat(
      $(this).parent().parent().attr("discount_percent")
    );
    var emiNo = parseFloat($(this).parent().parent().attr("emiNo"));
    console.log("editemiNo", emiNo);
    if (disPrecent > 0) {
      discount_type = 1;
  ...
  $("#disPercent").blur(function () {
    ...
    if (discount_type == 2) {
      calculateTotalItemWiseDiscount(
        discount,
        discount_type,
        discount_precent,
        discount_amount,
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (feedback != 1) {
            ...
            $("#saveItems").attr("disabled", true);
            $("#onlySaveItems").attr("disabled", true);
            $("#PrintSaveItems").attr("disabled", true);
            ...
            var invlink =
              baseUrl +
              "/Salesinvoice/view_sales_invoice_pdf/" +
              Base64.encode(invNumber);
            $("#lastInvoice").html(
              "<a href='" + invlink + "'>Last Invoice - " + invNumber + "</a>"
            );
            $("#btnPrint").attr("link", invlink);
  ...
  //===============save products without Payment Modal ==============================
...
var postData = {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (feedback != 1) {
            ...
            var invlink =
              baseUrl +
              "/Salesinvoice/view_sales_invoice_new/" +
              Base64.encode(invNumber);
            $("#lastInvoice").html(
              "<a href='" + invlink + "'>Last Invoice - " + invNumber + "</a>"
            );
  ...
  function calculateTotalItemWiseDiscount(
    discount,
    discount_type,
    disPercent,
    disAmount,
  ...
  $("#cart-pay-button").click(function () {
    ...
    console.log(
      ...
      parseFloat(before_advance_amount)
    );
  ...
  function strPad(input, length, string, code) {
    ...
    return input.length >= length
      ? code + input
      : code + new Array(length - input.length + 1).join(string) + input;
  }
  ...
  function loadSalesInvoiceDatatoGrid(resultData) {
    ...
    if (resultData.si_hed) {
      ...
      loadTotalVATNBT(isTotalVat, isTotalNbt, totalNbtRatio);
      var invlink =
        baseUrl + "/Salesinvoice/view_sales_invoice_new/" + Base64.encode(poNo);
      $("#lastInvoice").html(
        "<a href='" + invlink + "'>Last Invoice - " + poNo + "</a>"
      );
      $("#btnPrint").attr("link", invlink);
      ...
      for (var i = 0; i < resultData.si_dtl.length; i++) {
        ...
        $("#tbl_item tbody").append(
          "<tr ri=" +
            ...
            resultData.si_dtl[i].ProductCode +
            "' uc='" +
            resultData.si_dtl[i].SalesCaseOrUnit +
            "' qty='" +
            resultData.si_dtl[i].SalesQty +
            "' vatunit_price='" +
            ...
            resultData.si_dtl[i].SalesUnitPrice +
            "' upc='" +
            resultData.si_dtl[i].SalesUnitPerCase +
            "' caseCost='" +
            resultData.si_dtl[i].SalesCostPrice *
              resultData.si_dtl[i].SalesQty +
            "' isSerial='" +
            resultData.si_dtl[i].SalesSerialNo +
            "' serial='" +
            resultData.si_dtl[i].SalesSerialNo +
            "' discount_percent='" +
            resultData.si_dtl[i].SalesDisPercentage +
            "' cPrice='" +
            resultData.si_dtl[i].SalesCostPrice +
            "' pL='" +
            resultData.si_dtl[i].SalesPriceLevel +
            "' fQ='" +
            resultData.si_dtl[i].SalesFreeQty +
            "' nonDisTotalNet='" +
            resultData.si_dtl[i].SalesTotalAmount +
            "' netAmount='" +
            resultData.si_dtl[i].SalesInvNetAmount +
            "' proDiscount='" +
            resultData.si_dtl[i].SalesDisValue +
            "' proName='" +
            resultData.si_dtl[i].SalesProductName +
            "'   isvat='" +
            resultData.si_dtl[i].SalesIsVat +
            "' isnbt='" +
            resultData.si_dtl[i].SalesIsNbt +
            "' nbtRatio='" +
            resultData.si_dtl[i].SalesNbtRatio +
            "' proVat='" +
            resultData.si_dtl[i].SalesVatAmount +
            "' proNbt='" +
            resultData.si_dtl[i].SalesNbtAmount +
            "' salesPerson='" +
            resultData.si_dtl[i].SalesPerson +
            "' warranty='" +
            resultData.si_dtl[i].WarrantyMonthNew +
            "'>" +
            "' emiNo='" +
            resultData.si_dtl[i].emiNo +
            "'>" +
            "<td class='text-center'>" +
            (i + 1) +
            "</td>" +
            ...
            i +
            "'>" +
            accounting.formatNumber(resultData.si_dtl[i].SalesQty) +
            "</td>" +
            "<td class='text-right'>" +
            accounting.formatNumber(resultData.si_dtl[i].SalesFreeQty) +
            "</td>" +
            "<td class='text-right'>" +
            accounting.formatNumber(resultData.si_dtl[i].SalesUnitPrice) +
            "</td>" +
            "<td class='text-center'>" +
            resultData.si_dtl[i].SalesDisPercentage +
            "</td>" +
            "<td class='text-right' >" +
            accounting.formatMoney(resultData.si_dtl[i].SalesInvNetAmount -resultData.si_dtl[i].SalesDisValue) +
            "</td>" +
            "<td class='text-right' >" +
            resultData.si_dtl[i].SalesSerialNo +
            "</td>" +
            ...
            resultData.si_dtl[i].WarrantyMonthNew +
            "</td>" +
          "<td>" + (resultData.si_dtl[i].emiNo || "") + "</td>"
 +
            "<td><i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i></td><td class='rem" +
            i +
            "'><a href='#' class='remove btn btn-xs btn-danger'><i class='fa fa-remove'></i></a></td></tr>"
        );
      ...
      $("#regNo").val(resultData.si_hed.SalesVehicle);
      $("#netgrnamount").html(
        accounting.formatMoney(resultData.si_hed.SalesNetAmount)
      );
      $("#grndiscount").html(
        accounting.formatMoney(resultData.si_hed.SalesDisAmount)
      );
      $("#totalgrn").html(accounting.formatMoney(total_amount));
      $("#totalVat").html(
        accounting.formatMoney(resultData.si_hed.SalesVatAmount)
      );
      $("#totalNbt").html(
        accounting.formatMoney(resultData.si_hed.SalesNbtAmount)
      );
      $("#shipping").html(
        accounting.formatMoney(resultData.si_hed.SalesShipping)
      );
      $("#remark").val(resultData.si_hed.salesInvRemark);
    ...
    $("#customerName").html(resultData.cus_data.DisplayName);
    ...
    $("#lbltotalPOAmount").html(
      accounting.formatMoney(resultData.si_hed.SalesInvAmount)
    );
    $("#lbltotalDiscount").html(
      accounting.formatMoney(resultData.si_hed.SalesDisAmount)
    );
    $("#lbltotalVatAmount").html(
      accounting.formatMoney(resultData.si_hed.SalesVatAmount)
    );
    $("#lbltotalNbtAmount").html(
      accounting.formatMoney(resultData.si_hed.SalesNbtAmount)
    );
    $("#lbltotalPONetAmount").html(
      accounting.formatMoney(resultData.si_hed.SalesNetAmount)
    );
    $("#lbltotalPOAdvanceAmount").html(
      accounting.formatMoney(resultData.si_hed.SalesAdvancePayment)
    );

    
    $("#lbltotalPaidAmount").html(
      accounting.formatMoney(resultData.si_hed.SalesCustomerPayment)
    );
    $("#lbltotalBlanceAmount").html(
      accounting.formatMoney(resultData.si_hed.Blance)
    );

    if (resultData.si_hed.SalesCashAmount > 0) {
      $("#rowCash").hide();
      $("#lbltotalCashAmount").html(
        accounting.formatMoney(resultData.si_hed.SalesCashAmount)
      );
    } else {
      $("#rowCash").hide();
    }
    ...
    if (resultData.si_hed.SalesCCardAmount > 0) {
      $("#rowCard").show();
      $("#lbltotalCardAmount").html(
        accounting.formatMoney(resultData.si_hed.SalesCCardAmount)
      );
    } else {
      $("#rowCard").hide();
    }
    ...
    if (resultData.si_hed.SalesCreditAmount > 0) {
      $("#rowCredit").show();
      $("#lbltotalCreditAmount").html(
        accounting.formatMoney(resultData.si_hed.SalesCreditAmount)
      );
    } else {
      $("#rowCredit").hide();
    }
    ...
    if (resultData.si_hed.SalesChequeAmount > 0) {
      $("#rowCheque").show();
      $("#lbltotalChequeAmount").html(
        accounting.formatMoney(resultData.si_hed.SalesChequeAmount)
      );
    } else {
      $("#rowCheque").hide();
    }
    ...
    if (
      resultData.si_hed.SalesDisAmount > 0 ||
      resultData.si_hed.SalesNbtAmount > 0 ||
      resultData.si_hed.SalesVatAmount > 0
    ) {
      $("#rowNet").show();
    } else {
      $("#rowNet").show();
    ...
    if (resultData.si_dtl_arr) {
      $.each(resultData.si_dtl_arr, function (key, value) {
        ...
        if (resultData.si_hed.SalesInvType == 1 ||resultData.si_hed.SalesInvType == 3) {
          var i = 0;
         
          while (i < value.length) {
            var unitpric = value[i].SalesUnitPrice;
            var SalesQt = value[i].SalesQty;
            var SaleDisVal = value[i].SalesDisValue;
            var SalesDisPerc = value[i].SalesDisPercentage;
            ...
            if (value.length > 1 && value[i].SalesSerialNo != "") {
              ...
              if (value[0].type == null) {
                ...
                warrantyNew = value[0].type;
              }

              $("#tbl_po_data tbody").append(
                "<tr><td colspan='3'>" +
                  value[i].SalesProductName +
                  "<br>" +
                  value[i].SalesSerialNo +
                  "</td></tr>" +
                  "<tr>" +
                  ...
                  accounting.formatMoney(
                    parseFloat(value[0].SalesInvNetAmount)
                  ) +
                  "</td>" +
                  "<td style='text-align:right;padding: 3px;'>" +
                  value[0].SalesDisValue +
                  "</td>" +
                  "<td style='text-align:right;padding: 3px;'>" +
                  accounting.formatMoney(
                    parseFloat(
                      value[0].SalesInvNetAmount - value[0].SalesDisValue
                    )
                  ) +
                  "</td>" +
                  "</tr>"
              ...
              if (value[i].type == null) {
                ...
                warrantyNew = value[i].type;
              }

              let total = unitpric * SalesQt;
              let calcPercent = (SaleDisVal / total) * 100;

              if (Math.abs(calcPercent - SalesDisPerc) < 0.01) {
                  console.log('Disc Presentage',value[i].SalesDisValue);
                ...
                "<tr><td colspan='3'>" +
                  ...
                  "</td>" +
                  "<td style='text-align:right;padding: 3px;'>" +
                  value[i].SalesDisValue +
                  "</td>" +
                  "<td style='text-align:right;padding: 3px;'>" +
                  accounting.formatMoney(
                    value[i].SalesInvNetAmount - value[i].SalesDisValue
                  ) +
                  "</td>" +
                  "</tr>"
              );
              } else {
                    console.log('Disc Amount',value[i].SalesDisValue);
                ...
                "<tr><td colspan='3'>" +
                  ...
                  "</td>" +
                  "<td style='text-align:right;padding: 3px;'>" +
                  value[i].SalesDisValue* value[i].SalesQty +
                  "</td>" +
                  "<td style='text-align:right;padding: 3px;'>" +
                  accounting.formatMoney(
                    value[i].SalesInvNetAmount - value[i].SalesDisValue* value[i].SalesQty
                  ) +
                  "</td>" +
                  "</tr>"
              );
            ...
            if (value.length > 1 && value[i].SalesSerialNo != "") {
              ...
              $("#tbl_po_data tbody").append(
                "<tr><td style='padding: 3px;'>" +
                  value[0].SalesQty * value.length +
                  "</td><td style='padding: 3px;'>" +
                  value[0].SalesProductName +
                  "<br>" +
                  serial_no +
                  "</td><td style='text-align:right;padding: 3px;'>" +
                  value[0].WarrantyMonth +
                  "  Month(s) </td><td style='text-align:right;padding: 3px;'>" +
                  value[0].SalesUnitPrice +
                  "</td><td style='text-align:right;padding: 3px;'>" +
                  accounting.formatMoney(
                    parseFloat(value[0].SalesTotalAmount) * value.length
                  ) +
                  "</td></tr>"
              );
              i += value.length;
  ...
  function setGridandLabelData(data) {
    if (data.cus_data) {
      ...
      available_balance =
        parseFloat(data.cus_data.CreditLimit) - parseFloat(outstanding);
      ...
      $("#cusAddress").html(
        data.cus_data.Address01 + ", " + data.cus_data.Address02
      );
  ...
  function setPrintLabelData(data) {
    if (data.cus_data) {
      ...
      customer_name = data.cus_data.CusName;
      $("#lblcusName").html(
        data.cus_data.RespectSign + ", " + data.cus_data.CusName
      );
      $("#lbladdress1").html(
        nl2br(data.cus_data.Address01) + ", " + data.cus_data.Address02
      );
  ...
  function loadCustomerDatabyId(customer) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        available_balance =
          parseFloat(resultData.cus_data.CreditLimit) - parseFloat(outstanding);
        ...
        $("#cusName1").html(
          "<a href='" +
            encode_url +
            "'>" +
            resultData.cus_data.CusName +
            " " +
            resultData.cus_data.LastName +
            "</a>"
        );
        ...
        $("#cusAddress").html(
          resultData.cus_data.Address01 + ", " + resultData.cus_data.Address02
        );
  ...
  $("#serialNo").autocomplete({
    source: function (request, response) {
    console.log("Search Term (q):", request.term);
    console.log("Location:", loc);
    console.log("Row Number:", 1);
    console.log("Product Code:", itemCode);
  ...
  $("#emiNo").autocomplete({
    source: function (request, response) {
      ...
      $.ajax({
        url: baseUrl + "/Product/loadproductEmei",
        dataType: "json",
        data: {
          q: request.term,
          location: loc,
  ...
  function addPayment(
    ...
    dueAmount =
      totalNetAmount -
      parseFloat(
        pcash + pcard + pcheque + pcredit + padvance + pbank + preturn
      );
  ...
  $("#addCard").click(function () {
    ...
    if (dueAmount == 0) {
      if (camount > totalNet) {
        $("#errCard").show();
        $("#errCard")
          .html("You Cant Pay morethan Net Amount")
          .addClass("alert alert-danger alert-sm");
        $("#errCard").fadeOut(1500);
        return false;
      } else if (ctype == "" || ctype == 0) {
        $("#errCard").show();
        $("#errCard")
          .html("Please select a card type")
          .addClass("alert alert-danger alert-sm");
        $("#errCard").fadeOut(1500);
        return false;
        ...
        $("#errCard").show();
        $("#errCard")
          .html("Please enter card amount")
          .addClass("alert alert-danger alert-xs");
        $("#errCard").fadeOut(1500);
        return false;
      } else {
        if (ccTypeArrIndex < 0) {
          $("#tblCard tbody").append(
            "<tr ctype='" +
              ctype +
              "'  cref='" +
              cref +
              ...
              "</td><td><a href='#' class='btn btn-danger removeCard' ><i class='fa fa-close'></i></a></td></tr>"
          );
          ccard.push(ctype);
          cardAmount += camount;
          addPayment(
      ...
      if (camount > dueAmount) {
        $("#errCard").show();
        $("#errCard")
          .html("You Cant Pay morethan Due Amount")
          .addClass("alert alert-danger alert-sm");
        $("#errCard").fadeOut(1500);
        return false;
      } else if (ctype == "" || ctype == 0) {
    ...
    if (resultData.so_dtl != null || resultData.so_dtl != "") {
      ...
      for (var i = 0; i < resultData.so_dtl.length; i++) {
        if (resultData.so_dtl[i].ProType == 1) {
          if (resultData.so_dtl[i].ProCode > 0) {
            jobNumArr.push(resultData.so_dtl[i].ProCode);
          }
        } else if (resultData.so_dtl[i].ProType == 2) {
          if (resultData.so_dtl[i].ProCode > 0) {
            proCodeArr.push(resultData.so_dtl[i].ProCode);
          }
        } else if (resultData.so_dtl[i].ProType == 3) {
          if (resultData.so_dtl[i].ProCode > 0) {
            paintsArr.push(resultData.so_dtl[i].ProCode);
          }
        } else if (resultData.so_dtl[i].ProType == 4) {
          if (resultData.so_dtl[i].ProCode > 0) {
            parts2Arr.push(resultData.so_dtl[i].ProCode);
          }
        } else if (resultData.so_dtl[i].ProType == 5) {
          if (resultData.so_dtl[i].ProCode > 0) {
            parts3Arr.push(resultData.so_dtl[i].ProCode);
          }
        }
        ...
        // }
        $("#tbl_job tbody").append(
          "<tr est_price='0' discount_type='" +
            resultData.so_dtl[i].SalesDiscountType +
            "'  proDiscount='" +
            resultData.so_dtl[i].SalesDisValue +
            "' disPrecent='" +
            resultData.so_dtl[i].SalesDisPercentage +
            "'  totalPrice='" +
            resultData.so_dtl[i].SalesTotalAmount +
            "' isvat='" +
            resultData.so_dtl[i].SalesIsVat +
            "' isnbt='" +
            resultData.so_dtl[i].SalesIsNbt +
            "' nbtRatio='" +
            resultData.so_dtl[i].SalesNbtRatio +
            "' proVat='" +
            resultData.so_dtl[i].SalesVatAmount +
            "' proNbt='" +
            resultData.so_dtl[i].SalesNbtAmount +
            "'  job='" +
            resultData.so_dtl[i].SalesDescription +
            "' jobid='" +
            resultData.so_dtl[i].ProType +
            "' qty='" +
            resultData.so_dtl[i].SalesQty +
            "' jobOrder='" +
            resultData.so_dtl[i].SalesOrder +
            "' netprice='" +
            resultData.so_dtl[i].SalesNetAmount +
            "'  sellprice='" +
            resultData.so_dtl[i].SalesPrice +
            "'  isIns='" +
            resultData.so_dtl[i].SalesIsShow +
            "' insurance='" +
            resultData.so_dtl[i].SalesPrice +
            "' work_id='" +
            resultData.so_dtl[i].ProCode +
            "'  timestamp='" +
            resultData.so_dtl[i].SalesOrderTimestamp +
            "'><td>" +
            (i + 1) +
            "</td><td work_id='" +
            resultData.so_dtl[i].ProCode +
            "'>" +
            resultData.so_dtl[i].jobtype_name +
            "</td><td class='text-right'>" +
            resultData.so_dtl[i].SalesDescription +
            "</td><td class='text-right'>" +
            accounting.formatNumber(resultData.so_dtl[i].SalesQty) +
            "</td><td class='text-right'>" +
            accounting.formatNumber(resultData.so_dtl[i].SalesPrice) +
            "</td><td class='text-right'>" +
            accounting.formatNumber(resultData.so_dtl[i].SalesDisPercentage) +
            "</td><td class='text-right'>" +
            accounting.formatNumber(0) +
            "</td><td class='text-right'>" +
            accounting.formatNumber(resultData.so_dtl[i].SalesNetAmount) +
      ...
      totalVat = addTotalVat(
        resultData.so_hed.SOTotalAmount,
        resultData.so_hed.SOIsVatTotal,
        resultData.so_hed.SOIsNbtTotal,
        resultData.so_hed.SONbtRatioTotal
      );
      totalNbt = addTotalNbt(
        resultData.so_hed.SOTotalAmount,
        resultData.so_hed.SOIsVatTotal,
        resultData.so_hed.SOIsNbtTotal,
        resultData.so_hed.SONbtRatioTotal
