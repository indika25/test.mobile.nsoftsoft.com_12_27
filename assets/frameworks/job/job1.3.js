$(document).ready(function () {
  $("#appoDate,#deliveryDate").datetimepicker({
    dateFormat: "yy-mm-dd",
    timeFormat: "HH:mm:ss",
  });
  ...
  $("#newCustomer").click(function (e) {
    e.preventDefault();
    $("#customermodal .modal-content").load(
      "../customer/loadmodal_customeradd/",
      function (result) {
        $("#customermodal").modal({
          show: true,
          backdrop: "static",
  ...
  function addvehiclem(v) {
    $(".modal-content").load(
      "../customer/loadmodal_vehicleadd/" + v,
      function (result) {
  ...
  $("#saveJobform").submit(function (e) {
    ...
    var model = $("#phoneModel").val().trim();


    var emi = $("#emiNo").val().trim();
    ...
    if (cusCode == "" || cusCode == 0) {
     ...
     $.notify("Please add phone model.", "danger");
      $("#phoneModel").focus();
      return false;
    }else if (emi === "") {
    $.notify("Please add Emei Number", "danger");
    $("#emiNo").focus();
    return false;
}  else {
  ...
  function calculateTotal() {
    ...
    $("#tbl_payment tbody tr").each(function () {
      let amt = parseFloat($(this).find("td:eq(3)").text()) || 0;
      total += amt;
    });
  ...
  function invoicePrint(
    ...
    if (payType == 1) {
      $("#tblData tbody").append(
        "<tr><td colspan='3' style='border-left:#000 solid 1px;font-size:10px;border-right:#000 solid 1px;'>" +
          "Cash" +
          "</td><td style='text-align:right;border-right:#000 solid 1px;'>" +
          accounting.formatMoney(total) +
          "</td></tr>"
      );
    } else if (payType == 3) {
      $("#tblData tbody").append(
        "<tr><td style='border-left:#000 solid 1px;font-size:10px;border-right:#000 solid 1px;'>" +
          chq_date +
          "</td><td style='border-right:#000 solid 1px;text-align:left;'>" +
          chq_no +
          "</td><td style='border-right:#000 solid 1px;text-align:left;'>" +
          bank_name +
          "</td><td style='text-align:right;border-right:#000 solid 1px;'>" +
  ...
  function loadInvoicetoPrint(JobNo) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#lblAddress").html(
          nl2br(resultData.cus_data.Address01) +
            "<br>" +
            resultData.cus_data.Address02 +
            " " +
            resultData.cus_data.Address03
        );
        ...
        if (resultData.job_data.NextService > 0) {
          $("#lblNextService").html(
            parseFloat(resultData.job_data.OdoIn) +
              parseFloat(resultData.job_data.NextService)
          );
        }
        ...
        $("#lbldelveryDate").html(
          resultData.job_data.deliveryDate.substring(0, 10)
        );
        $("#lbldelveryTime").html(
          resultData.job_data.deliveryDate.substring(10, 19)
        );

        for (var i = 0; i < resultData.job_desc.length; i++) {
          k = i + 1;
          $("#tbl_jobcard_data tbody").append(
            "<tr><td style='text-align: center;'>" +
              k +
              "</td><td style='padding-left: 5px;'>" +
              resultData.job_desc[i].JobDescription +
              "</td><td>&nbsp;</td><td>&nbsp;</td></tr>"
          );
        }
        $("#tbl_jobcard_data tbody").append(
          "<tr><td style='height:150px;'></td><td style='height:150px;'></td><td style='height:150px;'></td><td style='height:150px;'></td></tr>"
        );
  ...
  function loadCustomerDatabyId(customer) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#customerName").html(
          "<a href='" + encode_url + "'>" + resultData.cus_data.CusName + "</a>"
        );
  ...
  function genarateJobView(job_no) {
    ...
    if (job_no != "") {
      $("#jobLink").attr(
        "href",
        "../../admin/job/view_job_card/" + Base64.encode(job_no)
      );
    }
  }
  ...
  function genarateInvLink(job_no) {
    ...
    if (job_no != "") {
      $("#invLink").attr(
        "href",
        "../../admin/Salesinvoice/job_invoice?type=job&id=" +
          Base64.encode(job_no)
      );
    }
  }
  ...
  function genarateEstLink(job_no) {
    ...
    if (job_no != "") {
      $("#estLink").attr(
        "href",
        "../../admin/job/estimate_job/" + Base64.encode(job_no)
      );
    }
  }
